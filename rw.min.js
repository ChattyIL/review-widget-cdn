// public/widget.js — review-widget v1.0.3 (no Make URL leaks)
(() => {
  const hostEl = document.getElementById("reviews-widget");
  if (!hostEl) return;

  // Shadow DOM (fallback to light DOM if not supported)
  const root = hostEl.attachShadow ? hostEl.attachShadow({ mode: "open" }) : hostEl;

  // Find the <script> tag that loaded us (async/CDN safe)
  const getScript = () =>
    document.currentScript ||
    [...document.querySelectorAll("script")].reverse().find(s =>
      s.hasAttribute("data-endpoint") || s.hasAttribute("data-slug")
    );

  const script = getScript();
  const explicitEndpoint =
    (script && script.getAttribute("data-endpoint")) ||
    hostEl.getAttribute("data-endpoint");

  const slug =
    (script && script.getAttribute("data-slug")) ||
    hostEl.getAttribute("data-slug");

  if (!explicitEndpoint && !slug) {
    root.innerHTML =
      '<div style="font-family: system-ui; color:#c00">Missing data-endpoint or data-slug.</div>';
    return;
  }

  // Default endpoint (uses your backend proxy, not make.com)
  const endpoint =
    explicitEndpoint ||
    (location.origin + "/api/reviews?client=" + encodeURIComponent(slug));

  const DEBUG = new URLSearchParams(location.search).get("rw_debug") === "1";

  // ---------- UI skeleton + styles (Assistant, RTL-friendly) ----------
  root.innerHTML = `
    <style>
      @import url('https://fonts.googleapis.com/css2?family=Assistant:wght@400;600;700&display=swap');
      :host{all:initial}*,*:before,*:after{box-sizing:border-box}
      .rw-wrap{position:fixed;right:24px;bottom:24px;z-index:2147483647;
               font-family:"Assistant",ui-sans-serif,system-ui;direction:rtl}
      ...
      .rw-header{font-weight:700;font-size:15px;margin:0 0 2px;color:#111;display:flex;align-items:center;gap:8px}
      ...
      .rw-text{
        ...
        line-height:1.4;color:#333;white-space:normal;font-size:15px;
  ...
  const firstNonEmpty = (...args) => {
    for (const v of args) if (v != null && String(v).trim() !== "") return String(v);
    return "";
  };
  function deepFind(obj, keyRegex, maxDepth=5){
    const seen = new Set();
    function walk(x, d){
      if (!x || typeof x!=='object' || d>maxDepth || seen.has(x)) return;
      seen.add(x);
  ...
  function normalizeReview(r = {}) {
    ...
    const author = firstNonEmpty(
      src.Header, deepFind(src, NAME_RE), deepFind(authorContainer, NAME_RE),
      ...
      src.title, src.name, "לקוח"
    );

    const rawText = firstNonEmpty(
      src.Content, deepFind(src, TEXT_RE),
      src.text, src.comment, src.review, src.content, src.description, src.snippet, ""
    );
    ...
    const photoUrl = firstNonEmpty(
      src.Photo, src.reviewerPhotoUrl, deepFind(src, PHOTO_RE), deepFind(authorContainer, PHOTO_RE), ""
    );
  ...
  function autoFitCard(card){
    ...
    if (!content) return;
    let fontSize = parseFloat(content.style.fontSize || 15) || 15;
    let clamp = parseInt(content.style.webkitLineClamp || 3, 10) || 3;
    const baseH = 120;
    const setHeights = () => {
  ...
  // ---------- card + rotation ----------
  const DISPLAY_MS = 10000, GAP_MS = 5000;
  function makeCard({author="לקוח", text="", rating=5, photoUrl}={}){
    ...
    const body = document.createElement("div"); body.className = "rw-body";
    const header = document.createElement("div"); header.className = "rw-header"; header.textContent = author;
    const content = document.createElement("div"); content.className = "rw-text"; content.textContent = text;
    ...
    const footer = document.createElement("div"); footer.className = "rw-footer";
    ...
    closeBtn.className = "rw-close"; closeBtn.type = "button";
    closeBtn.setAttribute("aria-label", "Close"); closeBtn.textContent = "×";
    closeBtn.onclick = () => exitCard(card);

    body.appendChild(header); body.appendChild(content); body.appendChild(footer);
    card.appendChild(closeBtn); card.appendChild(avatar); card.appendChild(body);
    return card;
  }
  function exitCard(card){
    if (!card || card.classList.contains("rw-exiting")) return;
    card.classList.add("rw-exiting"); setTimeout(() => card.remove(), 260);
  }
  ...
  function showNext(){
    ...
    list.appendChild(card); autoFitCard(card); showing = card;
    timers.push(setTimeout(() => { exitCard(card); timers.push(setTimeout(showNext, GAP_MS)); }, DISPLAY_MS));
  }

  // ---------- fetch + start ----------
  fetch(endpoint, { headers: { "Accept":"application/json" } })
    .then(r => r.json())
    .then(json => { if (DEBUG) console.log("[RW] raw", json);
    ...
    .catch(err => {
      ...
      reviews = normalizeArray(null); showNext();
    });
})();
