<!-- review-widget v1.10 — ES5-safe, golden stars, 20-word cap, "מאומת EVID" label -->
<script>
(function () {
  ...
  function log(){ if(DEBUG){ var a=["[reviews-widget v1.10]"]; for(var i=0;i<arguments.length;i++) a.push(arguments[i]); console.log.apply(console,a);} }
  ...
  style.textContent = ''
    ...
    + '.trust{margin-inline-start:auto;display:inline-flex;align-items:center;gap:6px;color:#111;font-weight:700;font-size:12px;}'
    + '.tick{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;border-radius:50%;background:#16a34a;color:#fff;font-size:12px;line-height:1;}'
  ...
  function mkStars(n){ var val=Math.max(0,Math.min(5,Math.round(Number(n)||0))); var out=[]; for(var i=0;i<5;i++) out.push(i<val?"★":"☆"); return out.join(" "); }
  function scaleClass(t){ t=(t||"").trim(); var len=t.length; if(len>220) return "tiny"; if(len>140) return "small"; return ""; }
  function truncateWords(s,n){ s=(s||"").replace(/\s+/g," ").trim(); var parts=s? s.split(" ") : []; return parts.length>n? parts.slice(0,n).join(" ")+"…" : s; }
  function colorFromString(s){ s=s||""; var h=0; for(var i=0;i<s.length;i++) h=(h*31+s.charCodeAt(i))>>>0; return "hsl("+(h%360)+" 70% 45%)"; }
  ...
  function renderCard(r){
    ...
    brand.innerHTML =
      ...
      + '<span class="trust"><span class="tick">✓</span><span>מאומת EVID</span></span>';
  ...
  fetch(endpoint,{method:"GET",credentials:"omit",cache:"no-store"})
    ...
    .catch(function(err){
      root.innerHTML =
        ...
        '</div>';
      console.error("[reviews-widget v1.10]", err);
    });
})();
</script>
