// both-controller v3.4.0 — ES5-safe, reviews + purchases with unified, pro UI.
// Purchases: RTL, product image on RIGHT, sentence to the LEFT of image, time in footer.
...
(function () {
  ...
  style.textContent = ''
    ...
    /* Purchases — RTL, image on RIGHT, text to its LEFT (headline a bit smaller) */
    + '.row-p{direction:rtl;grid-template-columns:1fr 82px 24px;gap:12px;}'
    + '.psentence{font-weight:700;font-size:15px;line-height:1.25;text-align:right;}'
    + '.pimg{justify-self:end;width:82px;height:82px;border-radius:14px;object-fit:cover;background:#eef2f7;display:block;border:1px solid rgba(2,6,23,.06);}'
    + '.pimg-fallback{justify-self:end;width:82px;height:82px;border-radius:14px;background:#e2e8f0;display:flex;align-items:center;justify-content:center;font-weight:700;color:#475569;}'
    + '.timebar{display:flex;align-items:center;gap:8px;justify-content:flex-start;padding:10px 12px;border-top:1px solid rgba(2,6,23,.07);font-size:12.5px;color:#475569;direction:rtl;}'
    ...
    /* Desktop boost for purchase image */
    + '@media (min-width: 720px){ .row-p{grid-template-columns:1fr 96px 24px;} .pimg,.pimg-fallback{width:96px;height:96px;} .psentence{font-size:15.5px;} }'
    ...
    + '  .row-p{grid-template-columns:1fr 72px 22px;gap:10px;padding:10px 10px 6px}'
    + '  .pimg,.pimg-fallback{width:72px;height:72px}'
  ...
  function renderPurchaseCard(p){
    ...
    // Preload product image to avoid blank pop
    var imgEl;
    ...
    if (p.image) {
      ...
      pre.onload = function(){
        var tag=document.createElement("img"); tag.className="pimg"; tag.alt=""; tag.src=p.image; replaceWith(tag);
      };
