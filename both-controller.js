// both-controller v3.3.4 — ES5-safe. Reviews use avatar (preloaded); Purchases use product image on RIGHT; time in footer.
(function () {
  ...
  function log(){ if (DEBUG) { var a=["[both-controller v3.3.4]"]; for (var i=0;i<arguments.length;i++) a.push(arguments[i]); console.log.apply(console,a);} }
  ...
  // shared review styles + purchase overrides
  ...
  function getPhotoUrl(o){
    ...
    var k=Object.keys(o);
    for(var i=0;i<k.length;i++){ var n=k[i], ln=n.toLowerCase();
  ...
  function normPurchase(x){
    return {
      kind:"purchase",
      buyer:   x.buyerName || x.buyer || x.customerName || x.name || x.customer || "לקוח/ה",
      product: x.productName || x.product || x.item || x.title || "מוצר",
      image:   x.productImage || x.image || x.product_img || "",
  ...
  function renderReviewCard(item){
    ...
    var body=document.createElement("div"); var shortText=truncateWords(item.text, MAX_WORDS);
    ...
    brand.innerHTML = '<span class="gmark" aria-label="Google">'
  ...
  function renderPurchaseCard(p){
    ...
    if (p.image) {
      ...
      pre.onload = function(){ var tag=document.createElement("img"); tag.className="pimg"; tag.alt=""; tag.src=p.image;
        if (imgEl && imgEl.parentNode) imgEl.parentNode.replaceChild(tag, imgEl); imgEl = tag; };
  ...
  function loadAll(){
    ...
    Promise.all([p1,p2]).then(function(r){ items = interleave(r[0]||[], r[1]||[]); start(); })
      .catch(function(e){ root.innerHTML='<div style="font-family: system-ui; color:#c00; background:#fff3f3; padding:12px; border:1px solid #f7caca; border-radius:8px">Widget error: '+ String(e && e.message || e) +'</div>'; });
  }
  loadAll();
})();
