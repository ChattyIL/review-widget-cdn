/*! both-controller v3.6.4 — Assistant no-FOUT, image prewarm, sticky review (mobile),
    ...
    + "קרא עוד" pill for long reviews (30+ words).
*/
(function () {
  ...
  // read-more threshold (words, using full text)
  var READ_MORE_WORD_THRESHOLD = 30;
  ...
  /* =========================
     ...
     ========================= */
  var ASSIST_HREF =
    "https://fonts.googleapis.com/css2?family=Assistant:wght@400;600;700&display=block";
  function ensureAssistantInHead() {
    try {
      if (!document.getElementById("asst-preconnect-goog")) {
        var pc1 = document.createElement("link");
        pc1.id = "asst-preconnect-goog";
        pc1.rel = "preconnect";
      ...
      if (!document.getElementById("asst-preconnect-gstatic")) {
        ...
        pc2.id = "asst-preconnect-gstatic";
        pc2.rel = "preconnect";
  ...
  style.textContent =
    ...
    /* Read-more pill (floating above name, outside header container) */
    + ".readmore-pill{position:absolute;top:-10px;right:82px;transform:translateY(-50%);background:#0f172a;color:#fff;border-radius:999px;padding:5px 10px;font-size:11px;font-weight:600;border:none;cursor:pointer;box-shadow:0 8px 22px rgba(15,23,42,.4);white-space:nowrap;z-index:4;}"
    ...
    + "  .hotcap{top:-10px;}"
    + "  .card.review-card .readmore-pill{right:76px;top:-6px;font-size:10px;padding:4px 8px;}"
    + "}"
  ...
  function colorFromString(s) {
    s = s || "";
    for (var h = 0, i = 0; i < s.length; i++) h = (h * 31 + s.charCodeAt(i)) >>> 0;
  ...
  function truncateForReview(text) {
    ...
    if (IS_MOBILE) {
      ...
      var words = t ? t.split(" ") : [];
      return words.length > MAX_WORDS_DESKTOP
        ? words.slice(0, MAX_WORDS_DESKTOP).join(" ") + "…"
        : t;
    }
  }
  ...
  function interleave(reviews, purchases) {
    ...
    while (i < reviews.length || j < purchases.length) {
      if (i < reviews.length) {
        out.push({ kind: "review", data: reviews[i++] });
      }
      if (j < purchases.length) {
        out.push({ kind: "purchase", data: purchases[j++] });
      }
    }
    return out;
  }
  ...
  function renderReviewCard(item) {
    ...
    name.className = "name";
    name.textContent = item.authorName || "Anonymous";
    ...
    // Read-more pill (only if review is longer than 30 words)
    if (wordCount(fullText) > READ_MORE_WORD_THRESHOLD) {
  ...
  function renderPurchaseCard(p) {
    ...
    textCol.className = "ptext";

    var tTop = document.createElement("div");
    tTop.className = "ptime-top";
    tTop.innerHTML =
    ...
    sentence.className = "psentence";
    var buyerFirst = firstName(p.buyer);
    sentence.innerHTML =
      '<strong class="buyer">' +
      escapeHTML(buyerFirst) +
      "</strong> רכש/ה " +
      '<span class="prod">' +
    ...
    function fb() {
      var d = document.createElement("div");
      d.className = "pimg-fallback";
      d.textContent = "✓";
      return d;
    }
    function swap(el) {
      if (imgEl && imgEl.parentNode) {
        imgEl.parentNode.replaceChild(el, imgEl);
      }
      imgEl = el;
    }
    if (p.image) {
    ...
    var hotcap = document.createElement("div");
    hotcap.className = "hotcap";
    hotcap.innerHTML =
  ...
  function showNext(overrideShowMs) {
    ...
    warmForItem(itm).then(function () {
      ...
      var card =
        itm.kind === "purchase" ? renderPurchaseCard(itm.data) : renderReviewCard(itm.data);
      wrap.innerHTML = "";
  ...
  function loadAll() {
    var p1 = REVIEWS_EP
      ? fetchJSON(REVIEWS_EP)
          .then(function (d) {
            var a = normalizeArray(d, "review");
            log("reviews:", a.length);
            return a;
          })
          .catch(function (e) {
            console.warn("reviews fetch err:", e);
            return [];
          })
      : Promise.resolve([]);
    ...
    var p2 = PURCHASES_EP
      ? fetchJSON(PURCHASES_EP)
          .then(function (d) {
            var a = normalizeArray(d, "purchase");
            log("purchases:", a.length);
            return a;
          })
          .catch(function (e) {
            console.warn("purchases fetch err:", e);
            return [];
          })
      : Promise.resolve([]);
    ...
    Promise.all([p1, p2])
      .then(function (r) {
        ...
        rev.forEach(function (v) {
          if (v.profilePhotoUrl) warmImage(v.profilePhotoUrl);
        });
        pur.forEach(function (v) {
          if (v.image) warmImage(v.image);
        });
      ...
      .catch(function (e) {
        root.innerHTML =
          '<div style="font-family: system-ui; color:#c00; background:#fff3f3; padding:12px; border:1px solid #f7caca; border-radius:8px">Widget error: ' +
